// ==============================================================================
// AQUAFLOW PRO - APPLICATION CONTROLLER & MAIN WORKSPACE BINDER
// ==============================================================================
...
class AttendanceApp {
  ...
  toggleTheme() {
    ...
    ToastService.info(`Switched to ${nextTheme.toUpperCase()} mode`);
  }
  ...
  async switchUser(userId) {
    ...
    this.updateApp();
    this.autoFitOnePage(false);
    ToastService.success(`Switched workspace to ${found.name}`);
  }
        ...
        if (email) {
          firebase.auth().signInWithEmailLink(email, window.location.href)
            .then((result) => {
              ...
              ToastService.success(`🎉 Passwordless login successful for ${email}!`);
            })
            .catch((err) => {
              console.error('Email Link Login Error', err);
              ToastService.error('Magic Link Sign In error: ' + err.message);
            });
        }
      }
    }
    ...
    if (typeof firebase !== 'undefined' && firebase.auth) {
      firebase.auth().getRedirectResult().then((result) => {
        if (result && result.user) {
          ...
          this.switchUser(userId);
          ToastService.success(`🌐 Logged in with Google as ${found.name}!`);
        }
      }).catch((err) => {
  ...
  bindEvents() {
      ...
      this.btnGoogleSignin.addEventListener('click', async () => {
        if (typeof firebase !== 'undefined' && firebase.auth) {
          try {
            ...
            ToastService.error('Google Sign In: ' + err.message);
          }
        } else {
          ToastService.info('Firebase Auth SDK initializing...');
        }
      });
    }
    ...
    if (this.btnMagicLink) {
      this.btnMagicLink.addEventListener('click', async () => {
        ...
        if (!emailVal || !emailVal.includes('@')) {
          ToastService.warning('Please enter a valid email address for Magic Link');
          return;
        }
        ...
        if (typeof firebase !== 'undefined' && firebase.auth) {
          try {
            ...
            ToastService.success(`✉️ Passwordless Magic Link sent to ${emailVal}! Check your inbox.`);
          } catch (err) {
            console.error('Magic Link Error:', err);
            ToastService.error('Magic Link error: ' + err.message);
          }
        }
      });
    }
        ...
        if (typeof firebase !== 'undefined' && firebase.auth && usernameVal.includes('@')) {
          try {
            ...
            if (userCred && userCred.user) {
              ...
              this.switchUser(userId);
              if (this.authModal) this.authModal.classList.remove('active');
              ToastService.success(`🔥 Logged in as ${found.name}`);
              return;
            }
          } catch (fbErr) {
    ...
    if (this.formRegister) {
      this.formRegister.addEventListener('submit', async (e) => {
        ...
        if (typeof firebase !== 'undefined' && firebase.auth && emailVal && passwordVal.length >= 6) {
          try {
            ...
            if (userCred && userCred.user) {
              ...
              this.switchUser(userId);
              if (this.authModal) this.authModal.classList.remove('active');
              ToastService.success(`🔥 Firebase account created for ${nameVal}!`);
              return;
            }
          } catch (fbErr) {
        ...
        this.switchUser(newId);
        if (this.authModal) this.authModal.classList.remove('active');
        ToastService.success(`Account created for ${nameVal}!`);
      });
    }
    ...
    if (this.btnLogout) {
      this.btnLogout.addEventListener('click', async () => {
        ...
        this.updateApp();
        if (this.authModal) this.authModal.classList.add('active');
        ToastService.info('Logged out. Switched to guest mode.');
      });
    }
    ...
    if (this.presetHeightBtns) {
      this.presetHeightBtns.forEach(btn => {
        btn.addEventListener('click', () => {
          ...
          this.saveActiveState();
          this.renderPreview();
          ToastService.info(`Row height set to ${h}px`);
        });
      });
    }
    ...
    this.categoryTabs.forEach(tab => {
      tab.addEventListener('click', () => {
        ...
        const samplePresets = window.SAMPLE_PRESETS || [];

        if (cat === 'Girls') {
          const match = presets.find(p => p.id === 'girls-batch-4' || p.category === 'Girls') || samplePresets[0];
          ...
          const match = presets.find(p => p.id === 'boys-batch-4' || p.category === 'Boys') || samplePresets[1];
        ...
        this.updateApp();
        this.autoFitOnePage(false);
        ToastService.info(`Loaded ${cat} student names list`);
      });
    });
    ...
    const addStudent = () => {
      ...
      if (name) {
        ...
        this.updateApp();
        this.autoFitOnePage(false);
        ToastService.success(`Added "${name}" to the list`);
      }
    };
    ...
    this.btnBulkAdd.addEventListener('click', () => {
      ...
      if (parsed.length > 0) {
        ...
        this.updateApp();
        this.autoFitOnePage(false);
        ToastService.success(`Added ${parsed.length} student names!`);
      }
    });
    ...
    this.btnSortNames.addEventListener('click', () => {
      ...
      this.updateApp();
      ToastService.info('Sorted student names alphabetically (A-Z)');
    });
    ...
    this.btnClearNames.addEventListener('click', () => {
      ...
      if (confirm('Are you sure you want to clear all student names?')) {
        this.state.names = [];
        this.updateApp();
        ToastService.info('Cleared all student names');
      }
    });
    ...
    this.btnSavePreset.addEventListener('click', () => {
      ...
      this.renderPresets();
      ToastService.success(`Name Preset "${presetName}" saved! (${preset.names.length} students)`);
    });
    ...
    this.btnExportJSON.addEventListener('click', () => {
      ...
      ToastService.success('Presets exported to JSON file');
    });
      ...
      reader.onload = (evt) => {
        try {
          ...
          if (Array.isArray(imported)) {
            ...
            this.saveStoredPresets();
            this.renderPresets();
            ToastService.success('Presets restored successfully!');
          }
        } catch (err) {
  ...
  loadPreset(id) {
    ...
    this.updateApp();
    this.autoFitOnePage(false);
    ToastService.info(`Loaded ${this.state.names.length} student names from "${preset.name}"`);
  }
  ...
  autoFitOnePage(showNotification = true) {
    ...
    this.renderPreview();
    if (showNotification) {
      ToastService.success(`Auto-fitted ${totalRows} rows to 1 page (${targetHeight}px/row)`);
    }
  }
